...
class BaseCache(object):
    ...
    def make_key(self, key, version=None):
        """Construct the key used by all other methods. By default it uses the
        key_func to generate a key (which, by default, prepends the
        `key_prefix' and 'version'). An different key function can be provided
    ...
    def get_many(self, keys, version=None):
        ...
        (memcached, pgsql) this can be *much* faster when fetching multiple
        values.

        Returns:
            dict: a mapping each key in keys to its value. If the given key is
            missing, it will be missing from the response dict.
        """
        d = {}
        for k in keys:
    ...
    def validate_key(self, key):
        ...
        This encourages (but does not force) writing backend-portable cache
        code.
        """
    ...
    def incr_version(self, key, delta=1, version=None):
        """Add delta to the cache version for the supplied key. Returns the
        new version.
        """
        if version is None:
            version = self.version
    ...
    def decr_version(self, key, delta=1, version=None):
        """Substract delta from the cache version for the supplied key.
